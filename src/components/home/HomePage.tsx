/*
rendered home page layout for logged in users with vertical sidebar navigation and main content area 
sidebar includes links to different sections of the home page (dashboard, map, learn, research, impact) with icons and labels
includes a theme toggle button, settings / profile links, logout button at the bottom
main content area includes a top bar with welcome message and user info & section for rendering the content of the currently active tab
animations are used for sidebar entrance, top bar entrance and page content transitions to create a smooth and engaging user experience
*/
...
/*
defines the structure of the props the HomeLayout component expects to receive from its parent component (UserDashboard): 
- onLogout: function called upon when the user clicks the logout button
- children: active homepage section component rendered
- activeTab: currently selected tab in sidebar
- setActiveTab: allows changing the active tab when user clicks on a different section in sidebar navigation
*/
interface HomeLayoutProps {
  onLogout?: () => void;
  children: React.ReactNode;
  activeTab: 'dashboard' | 'map' | 'learn' | 'research' | 'impact';
  setActiveTab: (tab: 'dashboard' | 'map' | 'learn' | 'research' | 'impact') => void;
}

// defines navigation items for homepage sidebar where each item has a label, icon & corresponding view identifier
const navItems = [
...
*/
export function HomePage({ onLogout, children, activeTab, setActiveTab}: HomeLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#071821] via-slate-950 to-[#071821] flex relative overflow-hidden">
      ...
      */}
      ...
      >
        ...
        */}
        <div className="p-4.5 border-b border-cyan-500/20 flex items-center justify-between">
          ...
          {/* Logo Section: 
          defines logo section of sidebar including icon & OceanIQ text (hidden when sidebar is collapsed)
          */}
      ...
      {/* Main Content Area
      acts as the container for the main content of the user dashboard, which changes based on the active tab selected in the sidebar 
