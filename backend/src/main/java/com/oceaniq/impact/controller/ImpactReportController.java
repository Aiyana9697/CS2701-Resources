...
public class ImpactReportController {
    ...
    // injecting ImpactReportService to handle business logic related to impact
    // reports
     ...
     * 
     *         creates a Pageable object containing pagination info (page number,
     *         size)
     *         calls service.getReports() to retrieve a Page of ImpactReportResponse
     *         based on filters and pagination info
     *         converts the Page<ImpactReportResponse> into a
     *         PaginatedResponse<ImpactReportResponse> which includes the list of
     *         reports + pagination metadata
     *         returns a successful (HTTP 200 OK) response with the paginated list
     *         of reports wrapped inside an ApiResponse object
    ...
    public ResponseEntity<ApiResponse<PaginatedResponse<ImpactReportResponse>>> getReports(
        ...
        Page<ImpactReportResponse> reports = service.getReports(search, impact, type, pageable);

        PaginatedResponse<ImpactReportResponse> response = new PaginatedResponse<>(
                reports.getContent(),
                reports.getNumber(),
     ...
     * 
     *         calls service.createReport() to create a new impact report based on
     *         the request data + ID of the authenticated user
     *         returns a successful (HTTP 200 OK) response with the created report
     *         wrapped inside an ApiResponse object
     *         which is serialized to JSON and returned to the client
     */
    @PostMapping
